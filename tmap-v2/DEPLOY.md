# Deploy AOF Code (web service)

ระบบนี้เป็นแบบ **key ผูกกับ account** → คุณ **ไม่ต้องใส่ provider key บน server**
server ต้องการแค่ 2 ความลับ: `JWT_SECRET` และ `AOF_MASTER_KEY` (host สร้างให้อัตโนมัติได้)
...
4. Render จะสร้าง `JWT_SECRET` / `AOF_MASTER_KEY` ให้อัตโนมัติ → กด **Apply**
5. ได้ URL เช่น `https://aof-code.onrender.com` → เปิด → Register → ใส่ key ของคุณ → Run
...
railway variables set AOF_MASTER_KEY=$(node -e "console.log(require('crypto').randomBytes(32).toString('hex'))")
```
...
```bash
docker build -t aof-code .
docker run -p 8787:8787 \
  -e JWT_SECRET=$(openssl rand -hex 32) \
  -e AOF_MASTER_KEY=$(openssl rand -hex 32) \
  -v aof-data:/app/.aof-server \
  aof-code
```
`-v aof-data:/app/.aof-server` ทำให้ข้อมูล user/key ไม่หายเมื่อรีสตาร์ต
...
- เก็บ `AOF_MASTER_KEY` ไว้ที่เดียวถาวร — ถ้าเปลี่ยน key นี้ จะถอดรหัส key เก่าของผู้ใช้ไม่ได้
