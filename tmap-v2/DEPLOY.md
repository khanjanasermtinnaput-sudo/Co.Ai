# Deploy Coagentix Platform (web service)

ระบบนี้เป็นแบบ **key ผูกกับ account** → คุณ **ไม่ต้องใส่ provider key บน server**
server ต้องการแค่ 2 ความลับ: `JWT_SECRET` และ `CGNTX_MASTER_KEY` (host สร้างให้อัตโนมัติได้)
...
4. Render จะสร้าง `JWT_SECRET` / `CGNTX_MASTER_KEY` ให้อัตโนมัติ → กด **Apply**
5. ได้ URL เช่น `https://coagentix.app` → เปิด → Register → ใส่ key ของคุณ → Run
...
railway variables set CGNTX_MASTER_KEY=$(node -e "console.log(require('crypto').randomBytes(32).toString('hex'))")
```
...
docker run -p 8787:8787 \
  ...
  -e CGNTX_MASTER_KEY=$(openssl rand -hex 32) \
  -v cgntx-data:/app/.coagentix-server \
  coagentix
```
`-v cgntx-data:/app/.coagentix-server` ทำให้ข้อมูล user/key ไม่หายเมื่อรีสตาร์ต
...
- เก็บ `CGNTX_MASTER_KEY` ไว้ที่เดียวถาวร — ถ้าเปลี่ยน key นี้ จะถอดรหัส key เก่าของผู้ใช้ไม่ได้
