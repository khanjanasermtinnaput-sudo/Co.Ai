...
const CHIEF_ANALYSIS_SYS = `You are the Chief Agent in Co.AI — an intelligent meta-orchestrator.
...
const SYNTHESIS_SYS = `You are the Chief Agent in Co.AI. You are synthesizing outputs from multiple specialized agents.
...
export async function runChiefAgent(
  ...
  if (agentOutputs.length === 0) {
    ...
    const directAnswer = await makeCall()([
      {
        role: 'system',
        content: 'You are Co.AI, a highly capable universal AI assistant. Answer thoroughly, accurately, and helpfully. Use markdown formatting.',
      },
      ...history.slice(-6),
      { role: 'user', content: expandedPrompt },
    ...
    const loopResult = await reviewLoop(
      ...
      async (critique) => {
        ...
        return makeCall()([
          {
            role: 'system',
            content: 'You are Co.AI. Revise and improve the following response based on the quality feedback. Write in the same language as the original response.',
          },
          {
            role: 'user',
            content: `Original request: ${userMessage}\n\nCurrent response:\n${finalResponse}\n\nQuality feedback:\n${critique}`,
