...
test('store + findImageByHash round-trips and dedups', async () => {
  const u = 'user-img-1';
  const rec = toRecord(u, PRODUCT, understanding('hash-aaa', 'Coagentix dashboard', 'Gemini quota 12 left'));
  await storeImageMemory(rec);

  const found = await findImageByHash(u, PRODUCT, 'hash-aaa');
  assert.ok(found);
  assert.equal(found!.shortSummary, 'Coagentix dashboard');

  // storing the same hash again must not create a duplicate
  await storeImageMemory(toRecord(u, PRODUCT, understanding('hash-aaa', 'Coagentix dashboard v2', 'Gemini quota 5 left')));
  const all = await listImageMemories(u, PRODUCT);
  assert.equal(all.filter((r) => r.imageHash === 'hash-aaa').length, 1);
});

test('findImageByHash returns undefined for unknown hash', async () => {
  const found = await findImageByHash('user-img-1', PRODUCT, 'nope');
  assert.equal(found, undefined);
});

test('findImageByHash does not cross product boundaries', async () => {
  const u = 'user-img-cross';
  await storeImageMemory(toRecord(u, 'cochat', understanding('hash-cross', 'cochat image', 'text')));
  const foundInCocode = await findImageByHash(u, 'cocode', 'hash-cross');
  assert.equal(foundInCocode, undefined);
  const foundInCochat = await findImageByHash(u, 'cochat', 'hash-cross');
  assert.ok(foundInCochat);
});

test('searchImageMemories ranks by relevance to the query', async () => {
  const u = 'user-img-2';
  await storeImageMemory(toRecord(u, PRODUCT, understanding('h1', 'invoice from Acme Corp', 'total 4500 baht due March', ['Acme Corp'])));
  await storeImageMemory(toRecord(u, PRODUCT, understanding('h2', 'cat photo in a garden', 'a fluffy cat outdoors', [])));

  const ranked = await searchImageMemories(u, PRODUCT, 'how much was the Acme invoice total', 3);
  assert.ok(ranked.length >= 1);
  assert.equal(ranked[0].record.imageHash, 'h1');
});

test('searchImageMemories returns nothing on no overlap', async () => {
  const ranked = await searchImageMemories('user-img-2', PRODUCT, 'quantum chromodynamics lagrangian', 3);
  assert.equal(ranked.length, 0);
});

test('imageMemoriesToContext renders an injectable block', async () => {
  const ranked = await searchImageMemories('user-img-2', PRODUCT, 'Acme invoice total', 3);
...
test('expired records are not returned', async () => {
  const u = 'user-img-3';
  const rec = toRecord(u, PRODUCT, understanding('expired-1', 'old screenshot', 'stale text'));
  rec.expiresAt = new Date(Date.now() - 1000).toISOString(); // already expired
  await storeImageMemory(rec);

  const all = await listImageMemories(u, PRODUCT);
  assert.equal(all.find((r) => r.imageHash === 'expired-1'), undefined);
  assert.equal(await findImageByHash(u, PRODUCT, 'expired-1'), undefined);
});

test('clearImageMemories empties the store', async () => {
  const u = 'user-img-4';
  await storeImageMemory(toRecord(u, PRODUCT, understanding('c1', 'thing', 'text')));
  assert.equal((await listImageMemories(u, PRODUCT)).length, 1);
  await clearImageMemories(u, PRODUCT);
  assert.equal((await listImageMemories(u, PRODUCT)).length, 0);
});
