# Coagentix — Technical Design Document (TMAP v2)
...
| DB (Supabase + file fallback) | ✅ DONE (5 tables) | `server/db.ts`, `supabase/migration.sql` | `users` · `memories` · `tmap_sessions` · `tmap_agent_logs` · `tmap_costs` — RLS เปิดแบบไม่มี policy (service-role only); file fallback ใน `.coagentix-server/db.json` / `/tmp` |
...
/v1/run {task,mode,planOnly?}
  ...
  → ถ้า planOnly → หยุดที่นี่ (Coagentix "Create Plan") ✅
...
`createBlackboard/logEvent/persist` ใน `core/blackboard.ts` คือ working memory ระดับ session — **เก็บไว้** แค่ย้าย `persist()` จากไฟล์ `.coagentix/` ไป DB/Redis (เพราะ Vercel `/tmp` หาย)

### 6.2–6.6 ที่ต้องเพิ่ม
- **Project/File Memory:** ตอน `co init`/index repo → chunk ตาม AST (function/class) → embed → เก็บ pgvector พร้อม `source_path`, `symbols`, `hash` (incremental ตาม git diff)
...
|---|---|---|---|
| `co doctor` | เช็ค key/agent resolve + project context | ✅ | `config.ts` + `gatherProjectContext` |
| `co agents` | role→model mapping | ✅ | `resolveAll` (ยังไม่โชว์ health — มี `/v1/health` ฝั่ง server) |
| `co context` | สแกน dir แสดง type/stack/deps/scripts/files | ✅ | `gatherProjectContext` |
| `co gencode(code/run) "<task>"` | รัน TMAP เต็มแล้ว gen ไฟล์ | ✅ | `runTMAP` (+`--apply` เขียนลง root, `--mode`) |
| `co titan ["<idea>"]` | Titan interactive: discovery→plan→approval→build | ✅ | `runTitan` → `blueprintToBuild` → `runTMAP` |
| `co review [dir]` | review โค้ดที่มีอยู่ (read-only, lite) | ✅ | `runTMAP` + Context Engine |
| `co fix [dir]` | gen fixes ให้ codebase เดิม | ✅ | `runTMAP` (+`--apply` overwrite in place) |
| `co sessions` | list local `.coagentix/sessions` ล่าสุด | ✅ | อ่าน session JSON |
| `co chat` | สนทนา/ถาม-ตอบ REPL | 🔴 | `/v1/chat` (RAA) SSE — server มีแล้ว ขาดฝั่ง CLI |
| `co explain <path>` | อธิบายโค้ด | 🔴 | single-agent + context |
| `co analyze` | ประเมิน brief/feasibility | 🔴 | `/v1/analyze` (server มีแล้ว) |
| `co build` | gen + validate + apply ครบ | 🔴 | TMAP `--apply` + sandbox |
| `co project` | init/index/สลับ project | 🔴 | `/v1/projects` (ยังไม่มี) |
| `co memory` | ดู/ค้น/ล้าง memory | 🔴 | `loadMemory`/`clearMemory` (server มี `/v1/memory`) |
| `co login` | username+PIN → เก็บ token | 🔴 | `/v1/auth/login` |
...
- `.coagentix/` ต่อ project (config + memory cache + conventions เทียบ `CLAUDE.md`)
- Windows-first: PowerShell, path Windows, แพ็กเป็น binary (`node --sea`) ติดตั้งผ่าน `npm i -g @coagentix/co-cli` (มี `bin: co` ใน package.json แล้ว ✅)
...
| Web | static terminal ✅ + **Next.js 14 (`aof-web/`)** ✅ | — |
