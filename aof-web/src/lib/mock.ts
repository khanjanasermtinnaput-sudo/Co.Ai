// ── Offline mock engine ───────────────────────────────────────────────────────
// Makes the whole Nexora experience work with zero backend / zero API keys, mirroring
...
export interface StreamHandlers {
  ...
  onError?: (error: NexoraProviderError) => void;
...
function composeChatReply(
  ...
  if (route.target === "code") {
    if (th) {
      const baseTh = `งานนี้เป็นงานวิศวกรรม ผมจะส่งต่อให้ **Nexora Code**`;
      if (style === "short") return `${pre}${baseTh} เปิด **Nexora Code** แล้วผมจะวางแผน สร้าง และรีวิวไฟล์ให้ครับ`;
      return `${pre}${baseTh}
...
อยากให้ผมสร้างเลยไหมครับ? เปิด **Nexora Code** แล้วผมจะพาจากไอเดียไปเป็นโค้ดที่ใช้งานได้`;
    }
    const base = `This is an engineering task, so I'd hand it to **Nexora Code**.`;
    if (style === "short") return `${pre}${base} Open **Nexora Code** and I'll plan, generate and review the files.`;
    return `${pre}${base}
...
Want me to build it? Open **Nexora Code** and I'll take it from idea to working code.`;
  }

  if (/(hello|hi|hey|สวัสดี|หวัดดี)/.test(m) && attachments.length === 0) {
    if (th) {
      return `สวัสดีครับ! ผมคือ **Nexora** — เวิร์กสเปซ AI ของคุณ ผมช่วยระดมไอเดีย ช่วยเรียนรู้ อ่านรูปภาพและ PDF หรือกระโดดเข้า **Nexora Code** เพื่อสร้างซอฟต์แวร์จริงได้ วันนี้อยากทำอะไรดีครับ?`;
    }
    return `Hi! I'm **Nexora** — your AI workspace. I can chat through ideas, help you learn, read images & PDFs, or jump into **Nexora Code** to build real software. What are we working on today?`;
  }
...
/** Stream a mock Nexora Code build log + summary (Lite / 1.0 / Pro). */
...
// ── Mock Nexora Code NORMAL_CHAT replies ────────────────────────────────────────

/** Mock reply for NORMAL_CHAT state in Nexora Code: greetings, tech Q&A, discussion.
...
// Keeps Nexora Code's conversation-first flow working with zero backend. It asks a
