...
export function toMarkdown(conv: Conversation): string {
  const lines: string[] = [
    `# ${conv.title}`,
    ``,
    `> Exported from Nexora · ${new Date(conv.createdAt).toLocaleString()}`,
    ``,
  ];

  for (const msg of conv.messages) {
    if (msg.role === "system") continue;
    const who = msg.role === "user" ? "**You**" : "**Nexora**";
 ...
 *  precedes a code block (the NEXORA_CODE_GEN_SYSTEM format uses **`path`**). */
...
/** Pull individual files out of Nexora Code's generated output. Handles every
 *  format the pipeline can produce:
 *   - ```path=src/main.js          (live tmap-v2 Coder, agents.ts)
 *   - **`src/index.ts`** + block   (serverless NEXORA_CODE_GEN_SYSTEM, raa.ts)
...
export function buildProjectHtml(files: ExtractedFile[]): string {
  ...
  if (!entry) {
    ...
    return [
      ...
      "<title>Nexora Code Preview</title>",
      styles,
      "</head>",
      "<body>",
      scripts,
...
async function buildZip(files: ExtractedFile[], brief?: ProjectBrief | null): Promise<Blob> {
  ...
  if (!hasReadme) {
    ...
    const name = brief?.project ?? "Nexora Code project";
    zip.file(
      "README.md",
      [
        `# ${name}`,
        "",
        `Generated by Nexora Code${kind !== "unknown" ? ` · ${kind} project` : ""}.`,
...
export async function exportProject(
  ...
  const slug = slugify(brief?.project ?? "nexora-code-project");
