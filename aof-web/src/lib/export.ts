...
export function toMarkdown(conv: Conversation): string {
  const lines: string[] = [
    `# ${conv.title}`,
    ``,
    `> Exported from Co.AI · ${new Date(conv.createdAt).toLocaleString()}`,
    ``,
  ];
...
/** Pull individual files out of CoCode's generated output. Handles every
...
async function buildZip(files: ExtractedFile[], brief?: ProjectBrief | null): Promise<Blob> {
  ...
  if (!hasReadme) {
    ...
    const name = brief?.project ?? "CoCode project";
    zip.file(
      "README.md",
      [
        `# ${name}`,
        "",
        `Generated by CoCode${kind !== "unknown" ? ` · ${kind} project` : ""}.`,
