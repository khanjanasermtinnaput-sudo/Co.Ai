...
test("missing-key hint → NEXORA_ERROR_001 with env var in solution", () => {
  const e = missingKeyError(P, "ANTHROPIC_API_KEY");
  assert.equal(e.code, "NEXORA_ERROR_001");
  assert.equal(e.problem, "API Key Missing");
  assert.match(e.details, /ANTHROPIC_API_KEY/);
  assert.match(e.solution, /ANTHROPIC_API_KEY/);
});

test("config hint → NEXORA_ERROR_013", () => {
  assert.equal(configError(P, "bad config").code, "NEXORA_ERROR_013");
});

test("empty hint → NEXORA_ERROR_011", () => {
  assert.equal(emptyResponseError(P).code, "NEXORA_ERROR_011");
});

test("timeout hint → NEXORA_ERROR_008", () => {
  assert.equal(classifyProviderError({ provider: P, hint: "timeout" }).code, "NEXORA_ERROR_008");
});

test("network hint → NEXORA_ERROR_007", () => {
  assert.equal(classifyProviderError({ provider: P, hint: "network" }).code, "NEXORA_ERROR_007");
});

// ── Classification by HTTP status ───────────────────────────────────────────

test("401 invalid → 002, expired → 003, generic → 010", () => {
  assert.equal(classifyProviderError({ provider: P, status: 401, message: "invalid x-api-key" }).code, "NEXORA_ERROR_002");
  assert.equal(classifyProviderError({ provider: P, status: 401, message: "token has expired" }).code, "NEXORA_ERROR_003");
  assert.equal(classifyProviderError({ provider: P, status: 401, message: "unauthorized" }).code, "NEXORA_ERROR_010");
});

test("403 quota → 004, generic → 010", () => {
  assert.equal(classifyProviderError({ provider: P, status: 403, message: "billing quota exceeded" }).code, "NEXORA_ERROR_004");
  assert.equal(classifyProviderError({ provider: P, status: 403, message: "forbidden" }).code, "NEXORA_ERROR_010");
});

test("404 → 009 (invalid model)", () => {
  assert.equal(classifyProviderError({ provider: P, status: 404, model: "claude-x" }).code, "NEXORA_ERROR_009");
});

test("429 rate-limit → 005, quota → 004", () => {
  assert.equal(classifyProviderError({ provider: P, status: 429, message: "too many requests" }).code, "NEXORA_ERROR_005");
  assert.equal(classifyProviderError({ provider: P, status: 429, message: "monthly quota exceeded" }).code, "NEXORA_ERROR_004");
});

test("400 with model → 009, otherwise → 012", () => {
  assert.equal(classifyProviderError({ provider: P, status: 400, message: "unknown model foo" }).code, "NEXORA_ERROR_009");
  assert.equal(classifyProviderError({ provider: P, status: 400, message: "bad request" }).code, "NEXORA_ERROR_012");
});

test("402 → 004 (out of credit)", () => {
  assert.equal(classifyProviderError({ provider: P, status: 402 }).code, "NEXORA_ERROR_004");
});

test("5xx → 006 provider unavailable", () => {
  for (const s of [500, 502, 503, 504]) {
    assert.equal(classifyProviderError({ provider: P, status: s }).code, "NEXORA_ERROR_006");
  }
});
...
test("network error messages → 007", () => {
  assert.equal(classifyProviderError({ provider: P, message: "fetch failed" }).code, "NEXORA_ERROR_007");
  assert.equal(classifyProviderError({ provider: P, message: "getaddrinfo ENOTFOUND api" }).code, "NEXORA_ERROR_007");
});

test("timeout messages → 008", () => {
  assert.equal(classifyProviderError({ provider: P, message: "ETIMEDOUT" }).code, "NEXORA_ERROR_008");
  assert.equal(classifyProviderError({ provider: P, message: "Request aborted: deadline" }).code, "NEXORA_ERROR_008");
});

test("anthropic error types map correctly", () => {
  assert.equal(classifyProviderError({ provider: P, errorType: "rate_limit_error" }).code, "NEXORA_ERROR_005");
  assert.equal(classifyProviderError({ provider: P, errorType: "authentication_error" }).code, "NEXORA_ERROR_002");
  assert.equal(classifyProviderError({ provider: P, errorType: "not_found_error" }).code, "NEXORA_ERROR_009");
  assert.equal(classifyProviderError({ provider: P, errorType: "insufficient_quota" }).code, "NEXORA_ERROR_004");
});

test("unrecognized failure → 012", () => {
  assert.equal(classifyProviderError({ provider: P, message: "??? something weird" }).code, "NEXORA_ERROR_012");
});
...
test("non-string error fields (numeric code) classify without throwing", () => {
  ...
  assert.ok(isNexoraProviderError(e));
  assert.equal(e.code, "NEXORA_ERROR_005"); // 429 → rate limit, no crash

  const typeOnly = { provider: P, errorType: 503 } as unknown as Parameters<typeof classifyProviderError>[0];
  assert.ok(isNexoraProviderError(classifyProviderError(typeOnly)));
});
...
test("every classified error carries the canonical fields", () => {
  ...
  assert.equal(e.kind, "nexora-provider-error");
...
test("problem matches the catalog for the code", () => {
  ...
  assert.equal(e.problem, ERROR_CATALOG[e.code as NexoraErrorCode].problem);
});
...
test("formatErrorBlock includes code and all fields", () => {
  ...
  assert.match(block, /NEXORA_ERROR_001/);
...
test("failover notice round-trips through the guard", () => {
  const n = makeFailoverNotice("Claude", "OpenRouter", "NEXORA_ERROR_004 · Quota Exceeded");
...
test("decodeFrames extracts an error frame and strips it from text", () => {
  ...
  assert.equal(r.errors[0].code, "NEXORA_ERROR_005");
});
...
test("failover frame then content, char-by-char, decodes cleanly", () => {
  const n = makeFailoverNotice("Claude", "OpenRouter", "NEXORA_ERROR_006 · Provider Unavailable");
