...
test("failure before the first token → ok:false with a classified error (no stream)", async () => {
  ...
  assert.equal(r.error?.code, "NEXORA_ERROR_002");
  assert.equal(r.error?.provider, PROVIDER_REGISTRY.anthropic.label);
});

test("provider closes with no content → NEXORA_ERROR_011 (empty response)", async () => {
  const r = await primeAndStream({ ctx, gen: yieldsNothing() });
  assert.equal(r.ok, false);
  assert.equal(r.error?.code, "NEXORA_ERROR_011");
});
...
test("prefix frame (failover) is emitted before the first token", async () => {
  const notice = makeFailoverNotice("Claude (Anthropic)", "OpenRouter", "NEXORA_ERROR_006");
...
test("mid-stream failure after content → in-band error frame, partial text preserved", async () => {
  ...
  assert.equal(decoded.errors[0].code, "NEXORA_ERROR_006");
});
...
// ── toNexoraError / isAbort ────────────────────────────────────────────────────

test("toNexoraError maps a ProviderHttpError by status", () => {
  assert.equal(toNexoraError(ctx, new ProviderHttpError(429, "{}", "rate_limit")).code, "NEXORA_ERROR_005");
  assert.equal(toNexoraError(ctx, new ProviderHttpError(404, "{}")).code, "NEXORA_ERROR_009");
  assert.equal(toNexoraError(ctx, new ProviderHttpError(503, "{}")).code, "NEXORA_ERROR_006");
});

test("toNexoraError treats connection-named errors as network", () => {
  const e = toNexoraError(ctx, { name: "APIConnectionError", message: "connect ECONNREFUSED" });
  assert.equal(e.code, "NEXORA_ERROR_007");
});
