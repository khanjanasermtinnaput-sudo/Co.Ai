...
const RELAY_SCRIPT = `<script>(function(){
  function send(level, parts){try{parent.postMessage({source:"nexora-preview",level:level,text:Array.prototype.slice.call(parts).map(function(p){
...
export function CodePreview({ buildLog, className }: { buildLog: string; className?: string }) {
  ...
  useEffect(() => {
    ...
    const onMessage = (e: MessageEvent) => {
      ...
      if (!d || d.source !== "nexora-preview") return;
      setLogs((prev) => [
        ...prev.slice(-199),
