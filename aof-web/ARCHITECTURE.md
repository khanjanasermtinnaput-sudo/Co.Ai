# Aof — Frontend Architecture

This document covers the deliverables for the Aof homepage experience: folder
structure, UI & component architecture, routing, state management, database
schema recommendations, and the implementation plan.

Aof is a **professional AI platform** — not a generic chatbot. It unifies three
products plus settings behind one premium, dark, orange-gold workspace:

| Product        | Route        | Notes                                              |
| -------------- | ------------ | -------------------------------------------------- |
| Chat with Aof  | `/`, `/chat` | The landing surface. Models: **Lite**, **Normal**. |
...
> **Titan is not a product.** It is the highest *mode inside Aof Code* and never
> appears on the homepage. It is only reachable from the Aof Code mode selector.
...
│   │       ├── page.tsx              # HOME → "Welcome to Aof" (Chat landing)
│   │       ├── chat/page.tsx         # Chat interface (Lite / Normal)
│   │       ├── code/page.tsx         # Aof Code workspace (+ Titan workflow)
...
│   ├── Logo / New Chat
│   ├── NavLink × (Chat, Projects, Aof Code, Settings)  — tooltips when collapsed
...
Aof Code so the core interaction feels identical everywhere.

## 4. Routing structure

App Router with a single `(app)` route group that owns the shell. The homepage
(`/`) **is** Chat with Aof — users land on a welcome + composer, never on Aof
...
- `code-store` — Aof Code `mode`, the build runner (Lite/1.0/Pro), and the full
...
6. ✅ Aof Code — Lite/1.0/Pro build view + the gated **Titan** workflow.
...
10. ⏭️ Next: wire auth (`/v1/auth/*`), persist conversations/projects to Supabase,
    stream live Aof Code file trees + diff view, and a one-click project export.
```
