...
-- Creates all tables needed for the Nexora Admin Dashboard.
...
-- Features: titan-beta | cli-beta | nexora-code-beta | experimental-models | early-access
...
-- show_on:  JSON array of "homepage" | "dashboard" | "chat" | "nexora-code"
...
create table if not exists public.api_usage_metrics (
  ...
  error_code        text,
  error_message     text,
  -- which Nexora feature triggered the request (e.g. "chat", "nexora-code", "titan")
  feature           text,
  -- route_target from the Nexora router (e.g. "chat", "code", "search")
  route_target      text,
  created_at        timestamptz not null default now()
);
