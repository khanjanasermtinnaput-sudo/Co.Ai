...
**Gap ที่ใหญ่ที่สุด 6 อย่าง (เรียงตามความสำคัญ):**
1. **DARS** — ระบบสลับ agent อัตโนมัติตอน runtime (ตอนนี้มีแค่ static fallback ตอน "ไม่มี key" ไม่มีการตรวจจับ API ล่ม/timeout/429/quota/latency/คุณภาพต่ำ ระหว่างทำงาน)
2. **Memory 6 ชั้น** — ตอนนี้ persist แค่ session JSON
3. **Voting/Consensus Engine** — ออกแบบไว้ใน TDD แต่ยังไม่มีในโค้ด
4. **Project Context Engine** — ยังไม่อ่านทั้งโปรเจกต์/วิเคราะห์ dependency
5. **CLI ครบ verb** — มีแค่ `doctor/agents/gencode`
6. **DB ครบ schema** — มีแค่ตาราง `users`
...
| **DARS (runtime failover)** | 🔴 TODO | — | มีแค่ static resolve fallback ตอนไม่มี key |
...
> **สถานะ: 🔴 TODO** — นี่คือ subsystem ใหม่ที่สำคัญที่สุด ออกแบบให้ห่อ (`wrap`) รอบ `chat()` และ `resolveRoleWith()` ที่มีอยู่ **โดยไม่ต้องแก้ orchestrator มาก**
...
| **Auth อ่อน** | ✅ | PIN hash (`hashPassword`) + JWT 30d. **ควรเพิ่ม:** rotate/refresh token, rate-limit login กัน brute-force PIN (PIN สั้น → จำกัด attempt!) |
| **Rate-limit / abuse** | 🔴 | ปัจจุบันไม่มี server rate-limit → เพิ่ม Redis token-bucket ต่อ user + quota เชิงต้นทุน (ผูก `tasks.cost_usd`) |
...
- [ ] `src/dars/` : health store (in-mem ก่อน), classify error, `chatWithDARS` wrap รอบ `chat()` (§4.5)
- [ ] เปลี่ยน `agents.ts` ให้เรียก `chatWithDARS` แทน `chat` (เปลี่ยนจุดเดียวต่อ agent)
- [ ] เพิ่ม timeout (`AbortController`) + retry + circuit breaker
- [ ] SSE emit `system: "X → switching agent"` (UI เดิมรองรับแล้ว)
- [ ] **Security เร่งด่วน:** rate-limit/lockout `/v1/auth/login` (PIN entropy ต่ำ)
- [ ] ตาราง `agent_logs` + `events` (§8) บันทึก failover
