...
| DB (Supabase + file fallback) | ✅ DONE (5 tables) | `server/db.ts`, `supabase/migration.sql` | `users` · `memories` · `tmap_sessions` · `tmap_agent_logs` · `tmap_costs` — RLS เปิดแบบไม่มี policy (service-role only); file fallback ใน `.aof-server/db.json` / `/tmp` |
...
/v1/run {task,mode,planOnly?}
  ...
  → ถ้า planOnly → หยุดที่นี่ (Aof Code "Create Plan") ✅
...
|---|---|---|---|
| `aof doctor` | เช็ค key/agent resolve + project context | ✅ | `config.ts` + `gatherProjectContext` |
| `aof agents` | role→model mapping | ✅ | `resolveAll` (ยังไม่โชว์ health — มี `/v1/health` ฝั่ง server) |
| `aof context` | สแกน dir แสดง type/stack/deps/scripts/files | ✅ | `gatherProjectContext` |
| `aof gencode(code/run) "<task>"` | รัน TMAP เต็มแล้ว gen ไฟล์ | ✅ | `runTMAP` (+`--apply` เขียนลง root, `--mode`) |
| `aof titan ["<idea>"]` | Titan interactive: discovery→plan→approval→build | ✅ | `runTitan` → `blueprintToBuild` → `runTMAP` |
| `aof review [dir]` | review โค้ดที่มีอยู่ (read-only, lite) | ✅ | `runTMAP` + Context Engine |
| `aof fix [dir]` | gen fixes ให้ codebase เดิม | ✅ | `runTMAP` (+`--apply` overwrite in place) |
| `aof sessions` | list local `.aof/sessions` ล่าสุด | ✅ | อ่าน session JSON |
| `aof chat` | สนทนา/ถาม-ตอบ REPL | 🔴 | `/v1/chat` (RAA) SSE — server มีแล้ว ขาดฝั่ง CLI |
| `aof explain <path>` | อธิบายโค้ด | 🔴 | single-agent + context |
| `aof analyze` | ประเมิน brief/feasibility | 🔴 | `/v1/analyze` (server มีแล้ว) |
| `aof build` | gen + validate + apply ครบ | 🔴 | TMAP `--apply` + sandbox |
| `aof project` | init/index/สลับ project | 🔴 | `/v1/projects` (ยังไม่มี) |
| `aof memory` | ดู/ค้น/ล้าง memory | 🔴 | `loadMemory`/`clearMemory` (server มี `/v1/memory`) |
